<template>
  <a-modal v-model:visible="modalVisible"
           :title="isAddEdit===AddEditEnum.ADD?'角色类型添加':'角色类型编辑'"
           title-align="start"
           width="550px"
           :mask-closable="false"
           @close="close">
...
import {roleTypeSave, roleTypeUpdate} from "../../../../common/api/system/role";

const emits = defineEmits(["queryRoleType"]);
...
const submit = () => {
  formRef.value.validate(async (errors:any)=>{
    ...
    if (coreTool.isUndefined(errors)) {
      const res:ResponseResult = (isAddEdit.value==AddEditEnum.ADD? await roleTypeSave(formData.value):await roleTypeUpdate(formData.value));
